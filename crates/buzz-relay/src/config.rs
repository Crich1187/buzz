...
pub fn handler_capacity_for_pool(db_pool_size: u32) -> usize {
    let usable = db_pool_size.saturating_sub(HANDLER_POOL_RESERVED_CONNECTIONS);
    usize::try_from(usable.max(1)).unwrap_or(usize::MAX)
}
...
mod tests {
    ...
    fn handler_capacity_reserves_headroom_under_the_pool() {
        ...
        // Small pools still yield a usable, non-zero capacity.
        assert_eq!(handler_capacity_for_pool(6), 1);
        assert_eq!(handler_capacity_for_pool(5), 1);
        assert_eq!(handler_capacity_for_pool(1), 1);
        // Never zero — a zero-permit semaphore would reject every request.
        for pool in 1..=64u32 {
            assert!(
                handler_capacity_for_pool(pool) >= 1,
